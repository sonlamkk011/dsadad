...
function ImageUploader({ isReset, defaultImagesArray, setcheckSaveButtonDisabled }: any) {
    ...
    useEffect(() => {
        if (defaultImagesArray) {
            setFileList(defaultImagesArray.map((image: any, index: any) => {
                return {
                    uid: index + 1,
    ...
    const handleChange = ({ fileList: newFileList }: any) => {
        ...
        if (newFileList.length) {
            setcheckSaveButtonDisabled((prev: boolean) => !prev);
        }
    }
