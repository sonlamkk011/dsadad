...
import { fetchChangeRealEstateStatus, getAllNewsRealEstateSearchFilter } from "../../../service/real-estate-all";
import { PurposeStatusMenu, RealEstateStatusMenu } from "../../../Hooks/DropdownMenu";
...
function AllRealEstates() {
    ...
    const getAllRealEstate = async () => {
        ...
        const response = await getAllNewsRealEstateSearchFilter(searchItems, setLoading, searchFilter, params);
        if (response) {
        ...
        },
        {
            ...
            render: (status: string, index: any) => {
                return (
                    <div key={index}>
                        {status === "SELL" ? (
                            <Tag color="blue" style={{ width: "100%" }} className="text-center">
                                <span className="text-uppercase">{`${t(status.toLowerCase())}`}</span>
                            </Tag>
                        ) : status === "RENT" ? (
                            <Tag color="purple" style={{ width: "100%" }} className="text-center">
                                <span className="text-uppercase">{`${t(status.toLowerCase())}`}</span>
                            </Tag>
                        ) : (
                            t("updating")
            ...
            render: (status: string, index: any) => {
                return (
                    <div id="real-estate-status" key={index}>
                        {status === "APPROVED" ? (
                            <Tag color="#5BD8A6">
                                <span className="text-uppercase status-tag">{`${t(status.toLowerCase())}`}</span>
                            </Tag>
                        ) : status === "PENDING" ? (
                            <Tag color="#FF9900">
                                <span className="text-uppercase status-tag">{`${t(status.toLowerCase())}`}</span>
    ...
    const showPromiseConfirm = () => {
        confirm({
            title: `Are you sure you want to ${actionType === "DELETED" ? "DELETE" : "REJECT"} ${selectedRowKeys.length > 1 ? "these real estate" : "this real estate"
                }?`,
                ...
                <div className="d-flex align-items-center justify-content-between" style={{ marginBottom: 16 }}>
                    ...
                    <span>
                        <Button href="/real-estate/create">
                            <div className="d-flex create-new-estate-btn">
