...
function ImageUploader({ isReset, defaultImages = [] }: any) {
    ...
    useEffect(() => {
        if (defaultImages && defaultImages.length > 0) {
            const defaultImagesArray: any = []
            defaultImages.forEach((image: string, index: any) => {
            ...
            setFileList(defaultImagesArray)
        }
    }, [isReset, defaultImages]);
    ...
    return (
        <>
            <Form.Item
                ...
                rules={[{ required: true, message: t("Please upload at least one image") }]}
            >
                <Upload
